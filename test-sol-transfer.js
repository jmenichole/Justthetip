...
async function testSOLTransfer() {
  console.log('🧪 JustTheTip SOL Transfer Test');
  console.log('================================');
  ...
  if (!senderAddress) {
    console.error('❌ FEE_PAYMENT_SOL_ADDRESS not set in .env');
    return;
  }
  ...
  console.log('📥 Test Recipient:', recipientPubkey.toString());
  console.log('💰 Amount: 0.01 SOL');

  try {
    // Check sender balance
    const senderBalance = await connection.getBalance(senderPubkey);
    console.log('📊 Sender Balance:', (senderBalance / 1000000000).toFixed(4), 'SOL');
    ...
    console.log('');
    console.log('🎯 To test with the Discord bot:');
    ...
    console.log('');
    console.log('📋 Transaction Data (Base64):');
    console.log(base64);
