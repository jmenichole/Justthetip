...
client.on('interactionCreate', async (interaction) => {
    try {
        if (interaction.isChatInputCommand()) {
            ...
            // Rate limiting
            if (!checkRateLimit(userId)) {
                return interaction.reply({ content: '⏳ Rate limit exceeded. Please wait before sending more commands.', ephemeral: true });
            }
            ...
            switch (commandName) {
                case 'airdrop': {
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .setDescription(`**${amount} ${coin}** is ready to be collected!`)
                        .addFields(
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId.startsWith('collect_')) {
                const airdropId = interaction.customId.split('_')[1];
                const airdrop = airdrops[airdropId];
                ...
                if (airdrop.collected.includes(userId)) {
                    return interaction.reply({ content: '❌ You have already collected this airdrop.', ephemeral: true });
                }
                
                // Add user to collected list
                airdrop.collected.push(userId);
                
                const embed = new EmbedBuilder()
                    .setColor('#10B981')
                    .setTitle('✅ Airdrop Collected!')
                    .setDescription(`You collected **${airdrop.amount} ${airdrop.coin}**!`)
                    .addFields(
                        { name: 'Amount', value: `${airdrop.amount} ${airdrop.coin}`, inline: true },
                        { name: 'From', value: `<@${airdrop.creator}>`, inline: true }
                    );
                
                await interaction.reply({ embeds: [embed], ephemeral: true });
                
                // Update original message to show collection
                const originalEmbed = new EmbedBuilder()
                    .setColor('#F59E0B')
                    .setTitle('🎁 Airdrop Status')
                    .setDescription(`**${airdrop.amount} ${airdrop.coin}** airdrop`)
                    .addFields(
                        { name: 'Creator', value: `<@${airdrop.creator}>`, inline: true },
                        { name: 'Collected by', value: `${airdrop.collected.length} user(s)`, inline: true },
                        { name: 'Latest', value: `<@${userId}>`, inline: true }
                    );
                ...
                try {
                    await interaction.editReply({ embeds: [originalEmbed], components: [disabledButton] });
                } catch (error) {
                    // Original message might not be editable, thats ok
                }
            }
        }
    } catch (error) {
