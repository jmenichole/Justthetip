...
export const Footer: React.FC<FooterProps> = ({ onOpenContactModal, onOpenPrivacyModal, onOpenTermsModal, onGoToAdmin }) => {
  return (
    <footer className="w-full max-w-5xl mx-auto py-8 text-gray-500 border-t border-white/10">
      <div className="flex flex-col sm:flex-row justify-between items-center text-center sm:text-left gap-8">
        ...
        <div className="text-xs text-gray-600 max-w-xs sm:text-right">
          ...
          <p className="mb-3">Disclaimer: This is a non-custodial helper tool. Users are responsible for their own wallet security and transactions.</p>
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 text-gray-500">
            <button onClick={onOpenPrivacyModal} className="hover:text-cyan-400 transition-colors duration-200">Privacy Policy</button>
            <button onClick={onOpenTermsModal} className="hover:text-cyan-400 transition-colors duration-200">Terms of Service</button>
          </div>
