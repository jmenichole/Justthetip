...
const { Client, GatewayIntentBits, SlashCommandBuilder, EmbedBuilder, REST, Routes, ActionRowBuilder, ButtonBuilder, ButtonStyle, AttachmentBuilder } = require('discord.js');
const fs = require('fs');
...
const { TaxTracker } = require('./src/utils/taxTracking.cjs');
...
const commands = [
    ...
    new SlashCommandBuilder()
        .setName('taxinfo')
        .setDescription('Get tax information and guidance for cryptocurrency transactions'),
    
    new SlashCommandBuilder()
        .setName('taxreport')
        .setDescription('Generate your annual tax report')
        .addIntegerOption(option =>
            option.setName('year')
                .setDescription('Tax year (default: current year)')
                .setRequired(false)),
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        if (action === 'collect') {
            ...
            // Record tax event for collector
            await taxTracker.recordTransaction(user.id, {
                type: 'collect',
                coin: airdrop.coin,
                amount: airdrop.amount,
                from: airdrop.creator,
            ...
            const successEmbed = new EmbedBuilder()
                ...
                .setFooter({ text: '📊 This collection is tracked for tax purposes' })
                .setTimestamp();
        ...
        switch (interaction.commandName) {
            case 'help':
                const helpEmbed = new EmbedBuilder()
                    ...
                    .setDescription('A non-custodial Discord crypto tipping bot with tax tracking')
                    .addFields(
                        ...
                        { name: '/taxinfo', value: 'Get tax guidance and information', inline: true },
                        { name: '/taxreport', value: 'Generate annual tax report (CSV)', inline: true },
                        { name: '/help', value: 'Show this help message', inline: true }
                    )
                    .setFooter({ text: 'Supported: SOL, USDC, LTC, ETH, XRP, TRX | 0.5% fee | Full tax tracking' })
                    .setTimestamp();
            ...
            case 'taxinfo':
                const taxGuidance = taxTracker.getTaxGuidanceMessage();
                const taxEmbed = new EmbedBuilder()
                    .setColor('#F59E0B')
                    .setTitle(taxGuidance.title)
                    .setDescription(taxGuidance.content)
                    .setFooter({ text: taxGuidance.footer })
                    .setTimestamp();
                
                await interaction.reply({ embeds: [taxEmbed], ephemeral: true });
                break;

            case 'taxreport':
                const reportYear = interaction.options.getInteger('year') || new Date().getFullYear();
                
                if (reportYear < 2020 || reportYear > new Date().getFullYear()) {
                    await interaction.reply({
                        content: '❌ Please specify a valid year between 2020 and the current year.',
                        ephemeral: true
                    });
                    return;
                }
                ...
                try {
                    const reportData = await taxTracker.exportTaxReportCSV(user.id, reportYear);
                    const report = reportData.report;
                    
                    const reportEmbed = new EmbedBuilder()
                        .setColor('#10B981')
                        .setTitle(`📊 Tax Report for ${reportYear}`)
                        .addFields(
                            { name: 'Total Transactions', value: report.totalTransactions.toString(), inline: true },
                            { name: 'Total Income (USD)', value: `$${report.totalIncomeUSD.toFixed(2)}`, inline: true },
                            { name: 'Total Fees Paid (USD)', value: `$${report.totalFeesUSD.toFixed(2)}`, inline: true }
                        )
                        .setFooter({ text: 'CSV file attached for detailed records. Consult a tax professional for advice.' })
                        .setTimestamp();
                    
                    // Add currency breakdown
                    let currencyBreakdown = '';
                    Object.entries(report.byCurrency).forEach(([currency, data]) => {
                        currencyBreakdown += `**${currency}**: ${data.transactionCount} transactions, $${data.totalFairValue.toFixed(2)} USD\n`;
                    });
                    
                    if (currencyBreakdown) {
                        reportEmbed.addFields({ name: 'By Currency', value: currencyBreakdown, inline: false });
                    }
                    
                    // Attach CSV file if it exists
                    let files = [];
                    if (fs.existsSync(reportData.filePath)) {
                        const attachment = new AttachmentBuilder(reportData.filePath, { 
                            name: `tax_report_${reportYear}.csv` 
                        });
                        files.push(attachment);
                    }
                    
                    await interaction.editReply({ embeds: [reportEmbed], files: files });
                    
                } catch (error) {
                    console.error('Tax report error:', error);
                    await interaction.editReply('❌ Failed to generate tax report. Please try again later.');
                }
                break;
            ...
            case 'balance':
                try {
                    ...
                    const balanceEmbed = new EmbedBuilder()
                        ...
                        .setFooter({ text: 'Balances tracked for convenience. All transactions recorded for tax purposes.' })
                        .setTimestamp();
            ...
            case 'tip':
                ...
                try {
                    ...
                    await db.addHistory(user.id, tipSentData);
                    await db.addHistory(recipient.id, tipReceivedData);
                    
                    // Record tax events
                    await taxTracker.recordTransaction(user.id, tipSentData);
                    await taxTracker.recordTransaction(recipient.id, tipReceivedData);
                    
                    // Record fee collection
                    if (tipFee > 0 && feeTxid) {
                        await taxTracker.recordFeeCollection({
                            coin: tipCoin,
                            fee: tipFee,
                            fromUserId: user.id,
                            txid: feeTxid,
                    ...
                    const tipEmbed = new EmbedBuilder()
                        ...
                        .setFooter({ text: '📊 Transaction recorded for tax purposes' })
                        .setTimestamp();
            ...
            case 'withdraw':
                ...
                try {
                    ...
                    await db.addHistory(user.id, withdrawData);
                    
                    // Record tax event
                    await taxTracker.recordTransaction(user.id, withdrawData);
                    
                    // Record fee collection
                    if (withdrawFee > 0 && feeTxid && withdrawFeeWallet) {
                        await taxTracker.recordFeeCollection({
                            coin: withdrawCoin,
                            fee: withdrawFee,
                            fromUserId: user.id,
                            txid: feeTxid,
                            feeWallet: withdrawFeeWallet
                        });
                    }
            ...
            case 'airdrop':
                ...
                await db.addHistory(user.id, airdropCreatedData);
                
                // Record tax event for airdrop creation
                await taxTracker.recordTransaction(user.id, airdropCreatedData);
                ...
                const airdropEmbed = new EmbedBuilder()
                    ...
                    .setFooter({ text: 'First come, first served! Collection tracked for tax purposes. ⚡' })
                    .setTimestamp();
